...
void run(int argc, char** argv)
{
    ...
    // Выбор API: OpenCL (вариант по умолчанию, работает на CPU).
    // Кернелы: src/kernels/cl/aplusb_matrix_bad.cl и src/kernels/cl/aplusb_matrix_good.cl
    ...
    // За один запуск кернела через память проходит 3 матрицы (2 чтения + 1 запись)
    double traffic_gb = 3.0 * sizeof(unsigned int) * width * height / 1024.0 / 1024.0 / 1024.0;

    {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            // Плохой кернел запущен на "перевернутом" рабочем пространстве height x width
            // (см. комментарий в src/kernels/cl/aplusb_matrix_bad.cl).
            // Рабочая группа GROUP_SIZE_X x GROUP_SIZE_Y = 16x16 = 256 work-items.
        ...
        // Достигнутая эффективная пропускная способность видеопамяти
        std::cout << "a + b BAD matrix kernel median VRAM bandwidth: " << traffic_gb / stats::median(times) << " GB/s" << std::endl;
...

    {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            // Хороший кернел запущен на прямом рабочем пространстве width x height.
            // Рабочая группа GROUP_SIZE_X x GROUP_SIZE_Y = 16x16 = 256 work-items.
        ...
        // Достигнутая эффективная пропускная способность видеопамяти
        std::cout << "a + b GOOD matrix kernel median VRAM bandwidth: " << traffic_gb / stats::median(times) << " GB/s" << std::endl;
