...
void run(int argc, char** argv)
{
...

    {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            gpu::WorkSize workSize(1, GROUP_SIZE, width / GROUP_SIZE_X, height);
        ...
        std::cout << "a + b BAD kernel median VRAM bandwidth: " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
...

    {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            // Обратите внимание что сейчас указана рабочая группа размера 1х1 в рабочем пространстве width x height, это не то что вы хотите
            // TODO И в плохом и в хорошем кернеле рабочая группа обязана состоять из 256 work-items
            gpu::WorkSize workSize(GROUP_SIZE, 1, width, height / GROUP_SIZE_Y);
        ...
        double memory_size_gb = sizeof(unsigned int) * 3 * width * height / 1024.0 / 1024.0 / 1024.0;
        std::cout << "a + b GOOD kernel median VRAM bandwidth: " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
