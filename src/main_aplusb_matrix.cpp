...
void run(int argc, char** argv)
{
...

    {
        ...
        for (size_t i = 0; i < width * height; ++i) {
            ...
            // rassert(cs[i] == 5, 321418230421312512, cs[i], as[i] + bs[i], i);
        }
    }
...

    {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            if (context.type() == gpu::Context::TypeOpenCL) {
                ...
                rassert(false, 468729876, context.type());
            }
        ...
        // Сверяем результат
        for (size_t i = 0; i < width * height; ++i) {
            rassert(cs[i] == as[i] + bs[i], 67349674896738967, cs[i], as[i] + bs[i], i);
            // rassert(cs[i] == 5, 67349674896738967, cs[i], as[i] + bs[i], i);
        }
    }
}
