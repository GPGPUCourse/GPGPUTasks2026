...
void run(int argc, char** argv)
{
    ...
    a_gpu.writeN(as.data(), cells);
    b_gpu.writeN(bs.data(), cells);

    {
        ...
        double memory_size_gb = sizeof(unsigned int) * 3 * cells / 1024.0 / 1024.0 / 1024.0;
        std::cout << "a + b kernel median VRAM bandwidth: " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;

        std::vector<unsigned int> cs(cells, 0);
        c_gpu.readN(cs.data(), cells);
