...
void run(int argc, char** argv)
{
...

    {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            // Показалось интересным, что если настроить рабочее пространство следующим образом:
            // `gpu::WorkSize workSize(1, GROUP_SIZE, width, height);`
            // и при этом кернел сделать идентичным кернелу *хорошей* матрицы, то окажется, что
            // эффект уменьшения bandwidth'а все равно достигнется, однако он будет не таким сильным,
            // как в случае *плохого* кернела (как сейчас).
            // Исходя их того, что получилось найти, это достигается за счет того, что порядок выдачи
            // блоков всегда один: blockIdx.x крутится быстрее всего, потом y.
        ...
        std::cout << "a + b BAD matrix kernel times (in seconds) - " << stats::valuesStatsLine(times) << std::endl;
        ...
        std::cout << "a + b BAD matrix kernel median VRAM bandwidth: " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
...

    {
        ...
        // Почти тот же код что с плохим кернелом, но теперь с хорошим
        std::vector<double> times;
        for (int iter = 0; iter < 10; ++iter) {
            timer t;
        ...
        std::cout << "a + b GOOD matrix kernel times (in seconds) - " << stats::valuesStatsLine(times) << std::endl;

        double memory_size_gb = sizeof(unsigned int) * 3 * width * height / (1024.0 * 1024.0 * 1024.0);
        std::cout << "a + b GOOD matrix kernel median VRAM bandwidth: " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
