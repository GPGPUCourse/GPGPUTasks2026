...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			cl_ulong mem_size = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong), &mem_size, nullptr));
			std::cout << "        Device memory size: " << mem_size / 1048576 << "MB" << std::endl;
			...
			if(available == CL_FALSE){
				std::cout << "        Device status: Unavailable" << std::endl;
			}else{
				std::cout << "        Device status: Available" << std::endl;
			}
