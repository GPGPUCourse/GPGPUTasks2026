...
int main()
{
	...
	for(cl_uint platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		// OCL_SAFE_CALL(clGetPlatformInfo(platform, 100500, 0, nullptr, &platformNameSize));
		...
		for(cl_uint deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			case CL_DEVICE_TYPE_ACCELERATOR:
				std::cout << "Dedicated OpenCL accelerator" << std::endl;
				std::cout << "            is OpenCL C supported: YES" << std::endl;
				break;
			case CL_DEVICE_TYPE_CUSTOM:
				std::cout << "Dedicated OpenCL accelerator" << std::endl;
				std::cout << "            is OpenCL C supported: NO" << std::endl;
				break;
			case CL_DEVICE_TYPE_DEFAULT:
				std::cout << "Default OpenCL device" << std::endl;
				break;
			default:
				std::cout << "UNKNOWN" << std::endl;
			}

			cl_ulong globalMemoryByte = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(globalMemoryByte), &globalMemoryByte, nullptr));
			std::cout << "        Memory size: " << globalMemoryByte / (1024 * 1024) << " MiB" << std::endl;

			cl_ulong cacheMemoryByte = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_CACHE_SIZE, sizeof(cacheMemoryByte), &cacheMemoryByte, nullptr));
			std::cout << "        Cache memory size: " << cacheMemoryByte / 1024 << " KiB" << std::endl;

			cl_ulong cacheLineMemoryByte = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE, sizeof(cacheLineMemoryByte), &cacheLineMemoryByte, nullptr));
			std::cout << "        Cache line memory size: " << cacheLineMemoryByte << " B" << std::endl;

			size_t devicePrintfBufferSize = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_PRINTF_BUFFER_SIZE, sizeof(devicePrintfBufferSize), &devicePrintfBufferSize, nullptr));
			std::cout << "        Device printf buffer size: " << devicePrintfBufferSize / (1024 * 1024) << " MiB" << std::endl;

			cl_uint deviceReferenceCount = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_REFERENCE_COUNT, sizeof(deviceReferenceCount), &deviceReferenceCount, nullptr));
			std::cout << "        Device reference count: " << deviceReferenceCount << std::endl;
