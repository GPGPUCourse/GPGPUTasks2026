...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		std::cout << "Platform name : " << std::string_view{infoBuf.data(), infoParamSize} << "\n";

		// TODO 1.3
		// Запросите и напечатайте так же в консоль вендора данной платформы
		OCL_SAFE_CALL(clGetPlatformInfo(platform, CL_PLATFORM_VENDOR, kInfoBufSize, bufPtr, &infoParamSize));
		std::cout << "Platform vendor : " << std::string_view{infoBuf.data(), infoParamSize} << "\n";
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
            ...
            std::cout << "Device name : " << std::string_view{infoBuf.data(), infoParamSize} << "\n";
