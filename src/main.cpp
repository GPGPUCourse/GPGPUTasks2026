...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		//OCL_SAFE_CALL(clGetPlatformInfo(platform, 41, 0, nullptr, &platformNameSize));
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			switch(deviceType)
			{
				...
				case CL_DEVICE_TYPE_DEFAULT: std::cout << "DEFAULT" << std::endl; break;
				case CL_DEVICE_TYPE_CUSTOM: std::cout << "CUSTOM" << std::endl; break;
				default: std::cout << "UNDEFINED TYPE" << std::endl; break;
			}
			...
			std::cout << "        Device memory size (Mb): " << deviceMemSize / (2 << 20) << std::endl;
			...
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_VENDOR, deviceVendor.size(), deviceVendor.data(), nullptr));
			std::cout << "        Device vendor: " << deviceVendor.data() << std::endl;

			cl_bool isDeviceLittleEndian;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_ENDIAN_LITTLE, sizeof(isDeviceLittleEndian), &isDeviceLittleEndian, nullptr));
			std::cout << "        Device endian: "<< (isDeviceLittleEndian ? "Little-endian" : "Big-endian") << std::endl;
			...
			std::cout << "        Device availability: "<< (isDeviceAvailable ? "True" : "False") << std::endl;
		}
	}
