...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_TYPE, sizeof(cl_device_type), &device_type, nullptr));
			...
			std::cout << "        Max memory allocation size: " << (maxMemAllocSize / (1024 * 1024)) << " MB" << std::endl;
