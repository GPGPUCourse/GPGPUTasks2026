...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		// Код ошибки -- -30, название -- CL_INVALID_VALUE
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			std::string typeStr;
			if(type & CL_DEVICE_TYPE_GPU) typeStr += "GPU";
			if(type & CL_DEVICE_TYPE_CPU) typeStr += "CPU";
			if(type & CL_DEVICE_TYPE_ACCELERATOR) typeStr += "Accelerator";
			...
			double globalMemMB = static_cast<double>(globalMemSize) / (1024.0 * 1024.0);
