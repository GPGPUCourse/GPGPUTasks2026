...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		std::cout << "Platfotm \"" << platformName.data() << "\" have number of devices is : " << devicesCount << std::endl;
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			const char *typeStr = "smth";
			switch(deviceType)
			{
			case CL_DEVICE_TYPE_CPU: typeStr = "CPU"; break;
			case CL_DEVICE_TYPE_GPU: typeStr = "GPU"; break;
			case CL_DEVICE_TYPE_ACCELERATOR: typeStr = "Accelerator"; break;
			case CL_DEVICE_TYPE_CUSTOM: typeStr = "Custom"; break;
			...
			size_t vendorSize = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_VENDOR, 0, nullptr, &vendorSize));
			std::vector<char> vendor(vendorSize, 0);
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_VENDOR, vendorSize, vendor.data(), &vendorSize));
			...
			std::vector<char> version(versionSize, 0);
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_VERSION, versionSize, version.data(), &versionSize));
			std::cout << "    Device version: " << version.data() << std::endl;
			...
			cl_uint maxClock = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(maxClock), &maxClock, nullptr));
			std::cout << "    Max clock frequency: " << maxClock << " MHz" << std::endl;
		}
	}
