...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			std::cout << "        Global memory: " << (globalMemorySize / (1024 * 1024)) << " MB" << std::endl;
			...
			cl_ulong globalMemoryCacheSize = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_CACHE_SIZE, sizeof(globalMemoryCacheSize), &globalMemoryCacheSize, nullptr));
			std::cout << "        Global memory cache: " << (globalMemoryCacheSize / 1024) << " KB" << std::endl;

			cl_uint globalMemoryCachelineSize = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE, sizeof(globalMemoryCachelineSize), &globalMemoryCachelineSize, nullptr));
			std::cout << "        Global memory cache line: " << globalMemoryCachelineSize << " bytes" << std::endl;
		}
	}
