...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		try {
			OCL_SAFE_CALL(clGetPlatformInfo(platform, 0x6767, 0, nullptr, &platformNameSize));
		} catch (const std::runtime_error& runtimeErr) {
			std::istringstream stream(runtimeErr.what());
			std::string first, second, third;
			int32_t code;

			if (stream >> first >> second >> third >> code &&
				first == "OpenCL" && second == "error" && third == "code" &&
				code == -30)
			{
				std::cerr << "Invalid platform name on OCL_SAFE_CALL!\n";
			} else {
				throw; // обработали только ошибку CL_INVALID_VALUE, остальное не обрабатываем
			}
		}
		...
		std::cout << "    		Devices avaliable for this platform: " << devicesCount << std::endl;

		for(int32_t deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			// TODO 2.2
			...
			if (deviceType & CL_DEVICE_TYPE_CUSTOM)
				std::cout << "Custom ";
			if (!(deviceType & (CL_DEVICE_TYPE_CPU | CL_DEVICE_TYPE_GPU | CL_DEVICE_TYPE_ACCELERATOR | CL_DEVICE_TYPE_CUSTOM)))
				std::cout << "Other (" << deviceType << ")";
			std::cout << std::endl;
			...
			std::cout << "    			Global memory (MiB): " << globalMemorySize / (1024.0 * 1024.0) << std::endl;
