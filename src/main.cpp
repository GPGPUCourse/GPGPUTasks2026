...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		// с параметром 239 возвращается код ошибки -30, в cl.h это соответствует CL_INVALID_VALUE, что в документации отсылает на некорректное значение param_name
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			std::cout << "            Size in MB: " << deviceMemSize / (1 << 20) << std::endl;
			...
			std::cout << "            Image support: " << (deviceImageSupport ? "true" : "false") << std::endl;

			cl_uint deviceMemCacheLine;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE, sizeof(deviceMemCacheLine), &deviceMemCacheLine, nullptr));
			std::cout << "            Cache line in bytes: " << deviceMemCacheLine << std::endl;
