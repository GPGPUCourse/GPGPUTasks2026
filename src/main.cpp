...
std::string_view getDeviceTypeName(cl_device_type deviceType) {
	switch (deviceType)
	{
...
int main()
{
	...
	// cl_platform_id *platformsList = (cl_platform_id*)malloc(1 * sizeof(cl_platform_id));
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			OCL_SAFE_CALL(clGetDeviceInfo(deviceID, CL_DEVICE_NAME, 0, 0, &deviceNameSize));
			...
			OCL_SAFE_CALL(clGetDeviceInfo(deviceID, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong), &deviceGlobalMemorySize, nullptr));
			std::cout << "        Global Memory device size: " << (deviceGlobalMemorySize / (cl_ulong)1000000) << " MB" << std::endl;
			...
			OCL_SAFE_CALL(clGetDeviceInfo(deviceID, CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(cl_uint), &maxClockFrequency, nullptr));
			std::cout << "        Maximum configured device clock frequency: " << maxClockFrequency << " MHZ" << std::endl;
			...
			*/ 
			...
			OCL_SAFE_CALL(clGetDeviceInfo(deviceID, CL_DEVICE_PROFILE, 0, 0, &deviceProfileSize));
			std::vector<unsigned char> deviceProfile(deviceNameSize, 0);
			OCL_SAFE_CALL(clGetDeviceInfo(deviceID, CL_DEVICE_PROFILE, deviceProfileSize, deviceProfile.data(), nullptr));
			std::cout << "        Device profile type: " << deviceProfile.data() << std::endl;
		}
	}
