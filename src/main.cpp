...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			else if (deviceType & CL_DEVICE_TYPE_GPU) deviceTypeStr = "GPU";
			else if (deviceType & CL_DEVICE_TYPE_ACCELERATOR) deviceTypeStr = "Accelerator";
			else if (deviceType & CL_DEVICE_TYPE_CUSTOM) deviceTypeStr = "Custom";
			else deviceTypeStr = "Unknown";
			std::cout << "        Device Type: " << deviceTypeStr << std::endl;

			// Размер памяти устройства в мегабайтах
			cl_ulong globalMem = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong), &globalMem, nullptr));
			std::cout << "        Global Mem Size: " << (globalMem / (1024 * 1024)) << "MiB" << std::endl;
			...
			std::cout << "        Device Image Support: " << (deviceImageSupport ? "TRUE" : "FALSE") << std::endl;
		}
	}
