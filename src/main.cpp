	...
	for(cl_uint platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(cl_uint deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			if (deviceType & CL_DEVICE_TYPE_CPU)
				deviceTypeStr += (deviceTypeStr.empty() ? "" : "|") + std::string("CPU");
			if (deviceType & CL_DEVICE_TYPE_GPU)
				deviceTypeStr += (deviceTypeStr.empty() ? "" : "|") + std::string("GPU");
			if (deviceType & CL_DEVICE_TYPE_ACCELERATOR)
				deviceTypeStr += (deviceTypeStr.empty() ? "" : "|") + std::string("Accelerator");
			if (deviceType & CL_DEVICE_TYPE_DEFAULT)
				deviceTypeStr += (deviceTypeStr.empty() ? "" : "|") + std::string("Default");
