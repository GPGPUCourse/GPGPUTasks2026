...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		try {
			...
			const std::string prefix = "OpenCL error code -30 encountered at";
			if (std::string(e.what()).substr(0, prefix.length()) != prefix)
				throw;
		}
		...
		{	
			...
			cl_uint deviceEndian;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_ENDIAN_LITTLE, sizeof(deviceEndian), &deviceEndian, nullptr));
			std::cout << "        Device endianness: " << (deviceEndian ? "Little Endian" : "Big Endian") << std::endl;
