...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			std::vector<unsigned char> NameDevice(deviceNameSize);
			OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_NAME, deviceNameSize, NameDevice.data(), nullptr));
			std::cout << "        Device name: " << NameDevice.data() << std::endl;
			...
			size_t deviceMemParamSize = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_GLOBAL_MEM_SIZE, 0, nullptr, &deviceMemParamSize));
			cl_ulong deviceMemSizeByte;
