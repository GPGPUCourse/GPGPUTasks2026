...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		const cl_int getDevicesCountResult = clGetDeviceIDs(platform, CL_DEVICE_TYPE_ALL, 0, nullptr, &devicesCount);
		if(getDevicesCountResult == CL_DEVICE_NOT_FOUND)
		{
			devicesCount = 0;
		}
		else
		{
			OCL_SAFE_CALL(getDevicesCountResult);
		}
		if(devicesCount == 0)
		{
			continue;
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			std::cout << "        Device memory size: " << deviceMemorySize / (1 << 20) << " MiB" << std::endl;

			// Число вычислительных единиц устройства
			...
			cl_bool deviceEndianness;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_ENDIAN_LITTLE, sizeof(deviceEndianness), &deviceEndianness, nullptr));
			std::cout << "        Device endianness: " << (deviceEndianness == CL_TRUE ? "little-endian" : "big-endian") << std::endl;
		}
	}
