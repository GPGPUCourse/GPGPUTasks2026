...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		OCL_SAFE_CALL(clGetPlatformInfo(platform, CL_PLATFORM_VENDOR, vendor_name_size, vendor_name.data(), nullptr));
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			cl_device_type devices_type;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_TYPE, sizeof(devices_type), &devices_type, nullptr));
			if(devices_type & CL_DEVICE_TYPE_GPU)
				std::cout << "    Type: GPU\n";
			else if(devices_type & CL_DEVICE_TYPE_CPU)
				std::cout << "    Type: CPU\n";
			...
			std::cout << "    Memory: " << (mem_size / MB) << "MB\n";

			cl_uint compute_units = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(compute_units), &compute_units, nullptr));
			std::cout << "    Compute units: " << compute_units << std::endl;

			size_t work_group = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(work_group), &work_group, nullptr));
			std::cout << "    Work group: " << work_group << std::endl;
			...
			std::cout << "    Clock frequency: " << max_freq << "MHz\n";

			size_t version_name_size = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_VERSION, 0, nullptr, &version_name_size));

			std::vector<unsigned char> version(version_name_size, 0);
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_VERSION, version_name_size, version.data(), nullptr));
