...
int main()
{
	...
	// Прочитайте документацию clGetPlatformIDs и убедитесь, что этот способ узнать, сколько есть платформ, 
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		// в документации подробно объясняется, какой ситуации соответствует данная ошибка, и это позволит,
		// проверив код, понять, чем же вызвана данная ошибка (некорректным аргументом param_name)
		// Обратите внимание, что в этом же libs/clew/CL/cl.h файле указаны всевоможные defines,
		// такие как CL_DEVICE_TYPE_GPU и т.п.

		// OCL_SAFE_CALL(clGetPlatformInfo(platform, 239, 0, nullptr, &platformNameSize));
		// код ошибки #define CL_INVALID_VALUE                            -30
		...
		// то есть важна вот эта строка, где мы планомерно все получаем, не очевидно, но все же
		...
		*/
		...
		// TODO 2.1
		// Запросите число доступных устройств данной платформы (аналогично тому, как это было сделано для запроса 
		// числа доступных платформ - см. секцию "OpenCL Runtime" -> "Query Devices")
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			// Global memory size
			// причем согласно доке, мемори сайз выдается в бутесах, а не мегабутесах
			...
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_PRINTF_BUFFER_SIZE, sizeof(printfBufferSize), &printfBufferSize, nullptr));
			std::cout << "        Printf buffer size: " << printfBufferSize / 1024 << " KB" << std::endl;
			...
			std::vector<char> driverVersion(driverVersionSize, 0);
