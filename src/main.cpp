...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			std::cout << "    	Memory size: " << (memSize >> 20)  << std::endl;

			// max sub devices
			cl_uint maxSubDevices;
			OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_PARTITION_MAX_SUB_DEVICES, sizeof(cl_uint), &maxSubDevices, nullptr));
			std::cout << "    	Max sub devices: " << maxSubDevices  << std::endl;

			// address bits
			cl_uint addressBits;
			OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_ADDRESS_BITS, sizeof(cl_uint), &addressBits, nullptr));
