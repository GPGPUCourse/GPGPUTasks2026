...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			std::cout << "    Device memory size: " << (deviceMemorySize >> 20)  << " MBytes" << std::endl;

			cl_uint deviceCacheLineSize;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE, sizeof(deviceCacheLineSize), &deviceCacheLineSize, nullptr));
			std::cout << "    Device cache line size: " << deviceCacheLineSize  << " bytes" << std::endl;
