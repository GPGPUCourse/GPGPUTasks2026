...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		// Решение 1.1: ввёл static_cast<cl_platform_info>(239) и вывело OpenCL error code -30 что соответствует CL_INVALID_VALUE т е if param_name is not one of the supported values
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			else if(deviceType & CL_DEVICE_TYPE_CUSTOM){std::cout << "Custom";}
			...
			std::cout << "    Global memory size: "<< globalMemorySize/(1024.0 * 1024.0) << " MB" << std::endl;
			...
			std::cout << "    Local memory size: "<< localMemorySize/(1024.0)<< " KB"<< std::endl;
			...
			cl_ulong maxMemoryAllocation = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_MAX_MEM_ALLOC_SIZE, sizeof(maxMemoryAllocation), &maxMemoryAllocation, nullptr));
			std::cout << "    Maximum memory allocation: " << maxMemoryAllocation/(1024.0 * 1024.0) << " MB" << std::endl;
			// Поддержка FMA
			cl_device_fp_config fpConfig = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_SINGLE_FP_CONFIG, sizeof(fpConfig), &fpConfig, nullptr));
			std::cout << "    FMA support: " << ((fpConfig & CL_FP_FMA) ? "yes" : "no") << std::endl;
		}
	}
