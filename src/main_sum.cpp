...
void run(int argc, char** argv)
{
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        write_times.push_back(t.elapsed());
    }
    std::cout << "CPU RAM -> GPU VRAM PCI-E bus bandwidth, median: " << (4 * n) / stats::median(write_times) / (1 << 30) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    ...
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        {
                            cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, reduction_n), input_gpu, reduction_buffer1_gpu, reduction_n);
                            reduction_n = (reduction_n + GROUP_SIZE - 1) / GROUP_SIZE;
                        }
                        {
                            cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, reduction_n), reduction_buffer1_gpu, reduction_buffer2_gpu, reduction_n);
                            reduction_n = (reduction_n + GROUP_SIZE - 1) / GROUP_SIZE;
                        }
                        {
                            cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, reduction_n), reduction_buffer2_gpu, reduction_buffer1_gpu, reduction_n);
                            reduction_n = (reduction_n + GROUP_SIZE - 1) / GROUP_SIZE;
                        }
                        {
                            cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, reduction_n), reduction_buffer1_gpu, sum_accum_gpu, reduction_n);
                            reduction_n = (reduction_n + GROUP_SIZE - 1) / GROUP_SIZE;
                        }
                        rassert(reduction_n == 1, 322263476437);
                        sum_accum_gpu.readN(&gpu_sum, 1);
                    } else {
                        rassert(false, 652345234321, algorithm, algorithm_index);
                    }
