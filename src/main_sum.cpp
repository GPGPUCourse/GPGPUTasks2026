...
void run(int argc, char** argv)
{
    ...
    constexpr unsigned TRIES_CNT = 5;
    std::vector<double> upload_times(TRIES_CNT);
    for (unsigned try_num = 0; try_num < TRIES_CNT; try_num++) {
        upload_times[try_num] = pci_time();
    }
    std::cout << "Input upload time: " << sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0 / stats::median(upload_times) << " GB/S" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        sum_accum_gpu.fill(0);
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, n), input_gpu, sum_accum_gpu, n);
                        sum_accum_gpu.readN(&gpu_sum, 1);
                    } else {
