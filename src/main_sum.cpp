...
void run(int argc, char** argv)
{
    ...
    std::cout << "PCI-E writeN times (in seconds) - " << stats::valuesStatsLine(pcie_times) << std::endl;
    double pcie_memory_size_gb = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;
    std::cout << "PCI-E writeN median bandwidth: " << pcie_memory_size_gb / stats::median(pcie_times) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    ...
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        while (cur_n > 1) {
                            cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, cur_n), *src, *dst, cur_n);
                            cur_n = div_ceil(cur_n, (unsigned int)GROUP_SIZE);
                            src = dst;
