...
void run(int argc, char** argv)
{
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        pcie_times.push_back(timer.elapsed());
    }
    std::cout << "PCI-E bandwidth: " << memory_size_gb / stats::median(pcie_times) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeCUDA) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        // reduction_buffer2_gpu - всегда результат редукции
                        for (unsigned int cur_size = n; cur_size > 1; cur_size = (cur_size + GROUP_SIZE - 1) / GROUP_SIZE) {
                            std::swap(reduction_buffer1_gpu, reduction_buffer2_gpu);
                            auto source = cur_size == n ? input_gpu : reduction_buffer1_gpu;
                            cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, cur_size), source, reduction_buffer2_gpu, cur_size);
                        }
