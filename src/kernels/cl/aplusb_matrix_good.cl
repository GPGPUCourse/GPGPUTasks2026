                     ...
                     unsigned int height)
{
    ...
    // ХОРОШИЙ вариант: work-item (x, y) считает элемент (x, y).
    // Соседние work-items по измерению 0 (соседние x) читают и пишут
    // соседние 4-байтовые элементы одной строки -> coalesced доступ:
    // каждая транзакция чтения/записи используется полностью.
