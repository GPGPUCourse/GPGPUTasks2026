                     ...
                     unsigned int height)
{
    ...
    const unsigned int start_row = index_y * GROUP_SIZE_Y;
    const unsigned int start_col = index_x;

    if (start_col >= width) {
        return;
    }

    for (unsigned int row = start_row; row < start_row + GROUP_SIZE_Y && row < height; ++row) {
       const unsigned int result_index = row * width + start_col;
       c[result_index] = a[result_index] + b[result_index];
    }
}
