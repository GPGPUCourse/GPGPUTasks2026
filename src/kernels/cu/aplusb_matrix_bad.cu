    ...
    unsigned int height)
{
    ...
    const unsigned int block_count = gridDim.x * gridDim.y;
    const unsigned int block_id = blockIdx.x * gridDim.y + blockIdx.y;
    const unsigned int element_id = threadIdx.x * blockDim.y + threadIdx.y;

    const unsigned int index = block_count * element_id + block_id;
